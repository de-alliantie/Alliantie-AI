...
Install the conda environment as described [below](#setup).
...
npm run build
```
and commit the build files so that the custom Streamlit component is up to date, if you made edits inside the `upload_component` folder. [See below](#streamlit-custom-component).
